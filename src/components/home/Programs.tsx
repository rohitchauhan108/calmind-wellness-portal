...
export default function Programs() {
  ...
  return (
    <section
      id="programs"
      className="py-24 sm:py-28 lg:py-32 bg-[#FBF9F5] relative overflow-hidden text-[#0D3C38]"
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div 
          ...
          style={{
            backgroundImage: `linear-gradient(to right, #0D3C38 1px, transparent 1px), linear-gradient(to bottom, #0D3C38 1px, transparent 1px)`,
        ...
        <svg
          className="absolute top-0 right-0 w-full h-full text-[#0D3C38]/10"
        ...
        >
          ...
          <circle cx="500" cy="550" r="3" fill="#0D3C38" fillOpacity="0.2" />
        </svg>
      </div>

      {/* Ambient Forest Color Glow Spheres */}
      <div className="absolute top-1/4 right-0 w-[450px] h-[450px] bg-[#0D3C38]/10 rounded-full blur-[150px] pointer-events-none" />
      <div className="absolute bottom-1/4 left-0 w-[350px] h-[350px] bg-[#0D3C38]/5 rounded-full blur-[120px] pointer-events-none" />

      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        {/* Section Header */}
        <div className="flex flex-col items-center text-center max-w-3xl mx-auto mb-16 space-y-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#0D3C38]/10 border border-[#0D3C38]/15 text-[#0D3C38] text-xs font-semibold tracking-widest uppercase">
            <Sparkles className="w-3.5 h-3.5 text-[#0D3C38]" />
            <span>Pathways of Growth</span>
          </div>

          <h2 className="text-3xl md:text-5xl font-serif text-[#0D3C38] tracking-tight leading-tight font-bold">
            Curated Programs for Lasting Transformation
          </h2>
          <p className="text-[#0D3C38]/80 font-normal text-base md:text-lg max-w-xl">
          ...
          <div className="pt-4 flex items-center gap-3">
            <span className={`text-sm font-semibold transition-colors ${!isVIP ? 'text-[#0D3C38]' : 'text-[#0D3C38]/60'}`}>
              Standard Individual
            </span>
            <button
              onClick={() => setIsVIP(!isVIP)}
              className="w-14 h-7 rounded-full bg-[#0D3C38]/15 p-1 flex items-center transition-colors duration-300 relative focus:outline-none cursor-pointer"
              aria-label="Toggle tier"
            >
              <motion.div
                layout
                className="w-5 h-5 rounded-full bg-[#0D3C38] shadow-sm"
            ...
            <span className={`text-sm font-semibold transition-colors ${isVIP ? 'text-[#0D3C38]' : 'text-[#0D3C38]/60'}`}>
              VIP High-Touch Tier{" "}
              <span className="text-xs bg-[#0D3C38]/10 text-[#0D3C38] px-2 py-0.5 rounded ml-1 font-semibold border border-[#0D3C38]/15">
                Priority Support
              </span>
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8" id="programs-grid">
          {programsList.map((prog, idx) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="bg-white/90 backdrop-blur-xl border border-[#0D3C38]/15 rounded-[32px] overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 flex flex-col justify-between"
            >
              <div>
                {/* Image & Header */}
                <div className="relative aspect-video overflow-hidden bg-[#0D3C38]/5">
                  <Image
                    src={prog.img}
                    alt={prog.title}
                    fill
                  ...
                  />
                  <div className="absolute top-4 left-4 z-10 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-semibold text-[#0D3C38] border border-[#0D3C38]/10 shadow-sm">
                    {prog.badge}
                  </div>
                  <div className="absolute top-4 right-4 z-10 bg-[#0D3C38]/85 backdrop-blur-md px-3 py-1 rounded-full text-xs font-semibold text-white shadow-sm flex items-center gap-1">
                ...
                <div className="p-8 space-y-6">
                  <div className="space-y-2">
                    <h3 className="text-xl md:text-2xl font-serif text-[#0D3C38] leading-tight font-bold">
                      {prog.title}
                    </h3>
                    <p className="text-[#0D3C38] font-semibold text-xs uppercase tracking-wider flex items-center gap-1.5">
                      <Calendar className="w-3.5 h-3.5" />
                      {prog.tagline}
                    </p>
                  </div>

                  <p className="text-[13.5px] text-[#0D3C38]/80 font-medium leading-relaxed">
                    {prog.desc}
                  </p>

                  <div className="space-y-3 pt-2">
                    <h4 className="text-[11px] uppercase tracking-wider font-bold text-[#0D3C38]">
                    ...
                    <div className="space-y-2">
                      {prog.features.map((feat, fIdx) => (
                        <div
                          key={fIdx}
                          className="flex items-start gap-2 text-xs font-medium text-[#0D3C38]/80 leading-snug"
                        >
                          <span className="w-4 h-4 rounded-full bg-[#0D3C38]/10 text-[#0D3C38] flex items-center justify-center shrink-0 mt-0.5">
              ...
              {/* Price & CTA Footer */}
              <div className="p-8 pt-0 border-t border-[#0D3C38]/10 mt-auto">
                <div className="flex items-baseline gap-1.5 py-6">
                  <span className="text-2xl font-serif font-bold text-[#0D3C38]">
                    ${isVIP ? prog.priceVIP.toLocaleString() : prog.priceIndividual.toLocaleString()}
                  </span>
                  <span className="text-xs text-[#0D3C38]/70 font-medium">
                    / USD program fee
                  </span>
                </div>
                ...
                <a
                  ...
                  className="w-full py-4 rounded-full bg-[#0D3C38] hover:bg-[#0D3C38]/90 text-white transition-colors duration-300 flex items-center justify-center gap-2 text-[14px] font-medium cursor-pointer shadow-md group"
                >
                  Enroll Now
