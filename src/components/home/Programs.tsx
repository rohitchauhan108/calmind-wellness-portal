...
export default function Programs() {
  ...
  return (
    <section id="programs" className="py-24 md:py-32 bg-brand-warm-white relative overflow-hidden">
      ...
      <div className="absolute top-1/4 right-0 w-[400px] h-[400px] bg-brand-brown-light/5 blur-[150px] pointer-events-none" />
      <div className="absolute bottom-1/4 left-0 w-[300px] h-[300px] bg-brand-olive-light/5 blur-[120px] pointer-events-none" />

      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        
        {/* Section Title */}
        <div className="flex flex-col items-center text-center max-w-3xl mx-auto mb-16 space-y-4">
          <span className="text-brand-olive text-sm font-semibold tracking-wider uppercase">Pathways of Growth</span>
          ...
          <p className="text-brand-charcoal-light font-light text-base md:text-lg max-w-xl">
          ...
          <div className="pt-4 flex items-center gap-3">
            <span className={`text-sm font-medium ${!isVIP ? 'text-brand-olive' : 'text-brand-charcoal-light'}`}>Standard Individual</span>
            <button
              onClick={() => setIsVIP(!isVIP)}
              className="w-14 h-7 rounded-full bg-brand-olive-light/20 p-1 flex items-center transition-colors duration-300 relative focus:outline-none"
              aria-label="Toggle tier"
            >
              <motion.div
                layout
                className="w-5 h-5 rounded-full bg-brand-olive shadow-sm"
                transition={{ type: 'spring', stiffness: 500, damping: 30 }}
                style={{ marginLeft: isVIP ? 'auto' : '0' }}
              />
            </button>
            <span className={`text-sm font-medium ${isVIP ? 'text-brand-olive' : 'text-brand-charcoal-light'}`}>
              VIP High-Touch Tier <span className="text-xs bg-brand-olive/10 text-brand-olive px-2 py-0.5 rounded ml-1 font-semibold">Priority Support</span>
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8" id="programs-grid">
          {programsList.map((prog, idx) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="bg-white border border-brand-olive/5 rounded-[32px] overflow-hidden shadow-premium hover:shadow-premium-hover transition-all duration-300 flex flex-col justify-between"
            >
              <div>
                {/* Image & Header */}
                <div className="relative aspect-video overflow-hidden bg-brand-beige">
                  ...
                  />
                  <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-semibold text-brand-olive-dark shadow-sm">
                    {prog.badge}
                  </div>
                  <div className="absolute top-4 right-4 bg-brand-charcoal/80 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-semibold text-brand-cream shadow-sm flex items-center gap-1">
                ...
                <div className="p-8 space-y-6">
                  <div className="space-y-2">
                    <h3 className="text-xl md:text-2xl font-serif text-brand-charcoal leading-tight">
                      {prog.title}
                    </h3>
                    <p className="text-brand-olive font-medium text-xs uppercase tracking-wider flex items-center gap-1.5">
                      <Calendar className="w-3.5 h-3.5" />
                      {prog.tagline}
                    </p>
                  </div>

                  <p className="text-[13.5px] text-brand-charcoal-light font-light leading-relaxed">
                    {prog.desc}
                  </p>

                  <div className="space-y-3 pt-2">
                    <h4 className="text-[11px] uppercase tracking-wider font-semibold text-brand-charcoal">Comprehensive Inclusions:</h4>
                    <div className="space-y-2">
                      {prog.features.map((feat, fIdx) => (
                        <div key={fIdx} className="flex items-start gap-2 text-xs font-light text-brand-charcoal-light leading-snug">
              ...
              {/* Price & CTA Footer */}
              <div className="p-8 pt-0 border-t border-brand-olive/5 mt-auto">
                <div className="flex items-baseline gap-1.5 py-6">
                  <span className="text-2xl font-serif font-semibold text-brand-charcoal">
                    ${isVIP ? prog.priceVIP.toLocaleString() : prog.priceIndividual.toLocaleString()}
                  </span>
                  <span className="text-xs text-brand-charcoal-light font-light">
                    / USD program fee
                  </span>
                </div>
                ...
                <button 
                  ...
                  className="w-full py-4 rounded-full bg-brand-olive hover:bg-brand-olive-dark text-brand-cream transition-colors duration-300 flex items-center justify-center gap-2 text-[14px] font-medium cursor-pointer shadow-sm group"
                >
