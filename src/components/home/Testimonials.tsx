...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="py-12 bg-brand-cream relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-brand-brown-light/5 blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="space-y-3 max-w-2xl">
            <span className="text-brand-olive text-xs font-semibold tracking-wider uppercase">
              Voices of Serenity
            </span>
            <h2 className="text-3xl md:text-5xl font-serif text-brand-charcoal tracking-tight leading-tight">
              Client Experiences
            </h2>
            <p className="text-brand-charcoal-light font-light text-sm md:text-base">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <button
              onClick={handlePrev}
              className="w-12 h-12 rounded-full border border-brand-olive/20 hover:border-brand-olive bg-white hover:bg-brand-olive/5 text-brand-charcoal transition-all duration-300 flex items-center justify-center cursor-pointer shadow-sm active:scale-95"
            ...
            <button
              onClick={handleNext}
              className="w-12 h-12 rounded-full bg-brand-olive hover:bg-brand-olive-dark text-brand-cream transition-all duration-300 flex items-center justify-center cursor-pointer shadow-md active:scale-95"
            ...
            <AnimatePresence mode="popLayout" initial={false}>
              {visibleCards.map((item, idx) => (
                <motion.div
                  ...
                  className="bg-white border border-brand-olive/10 rounded-3xl p-8 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between relative group overflow-hidden"
                >
                  {/* Decorative Subtle Top Accent Line */}
                  <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-brand-olive/20 to-transparent group-hover:via-brand-olive transition-all duration-500" />
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Quote className="w-6 h-6 text-brand-olive/20 group-hover:text-brand-olive/40 transition-colors" />
                    </div>

                    {/* Testimonial Text */}
                    <p className="text-sm font-serif font-light text-brand-charcoal leading-relaxed italic">
                      "{item.quote}"
                    </p>
                  </div>

                  {/* Author Footer */}
                  <div className="pt-6 mt-6 border-t border-brand-olive/10 flex items-center gap-3">
                    <img
                      src={item.avatar}
                      alt={item.author}
                      referrerPolicy="no-referrer"
                      className="w-11 h-11 rounded-full object-cover border border-brand-olive/15 shrink-0"
                    />
                    <div className="overflow-hidden">
                      <h4 className="font-serif text-brand-charcoal text-sm font-semibold truncate">
                        {item.author}
                      </h4>
                      <p className="text-[11px] text-brand-charcoal-light font-light truncate">
                        {item.title}
                      </p>
                      <span className="inline-block text-[9px] uppercase font-bold text-brand-olive bg-brand-olive/5 px-2 py-0.5 rounded mt-1 truncate">
                        {item.program}
                      </span>
                    </div>
                  </div>
            ...
            {list.map((_, idx) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-300 ${
                  currentIndex === idx ? 'w-8 bg-brand-olive' : 'w-2 bg-brand-olive/20'
                }`}
